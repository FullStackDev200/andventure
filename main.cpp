...
int main() {
  ...
  // Create a rectangle and set its size based on the second room
  if (!sfmlRooms.empty()) { // Check if there are rooms in the list
    ...
    rectangle.setSize(
        sf::Vector2f(rooms[0].first,
                     rooms[0].second));        // Make sure these are reasonable
